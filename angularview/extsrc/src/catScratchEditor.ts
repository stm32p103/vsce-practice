...
export class CatScratchEditorProvider implements vscode.CustomTextEditorProvider {
	...
	public async resolveCustomTextEditor(
		...
		console.log(webviewPanel.webview.options  );
		...
		webviewPanel.webview.onDidReceiveMessage(e => {
			...
			console.log( e );
			webviewPanel.webview.postMessage( { title: 'bye' } ).then( ()=>{ console.log('hi')} );
		}
		);
	...
	private getHtmlForWebview(webview: vscode.Webview): string {
		...
		const htmlDir = path.join( this.context.extensionPath, 'view' );
		const htmlPath = path.join( htmlDir, 'index.html' );
		let html = fs.readFileSync( htmlPath, { encoding: 'utf-8' } );

		// Use a nonce to whitelist which scripts can be run
    	const baseUri = webview.asWebviewUri(vscode.Uri.file(htmlDir));
		html = html.replace('<base href="./">', `<base href="${String(baseUri)}/">`);
